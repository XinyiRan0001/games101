...
static bool insideTriangle(float x, float y, const Vector3f* _v)
{
    ...
    Eigen::Vector3f a(_v[0].x(), _v[0].y(), 1.0f);
    Eigen::Vector3f b(_v[1].x(), _v[1].y(), 1.0f);
    Eigen::Vector3f c(_v[2].x(), _v[2].y(), 1.0f);
...
void rst::rasterizer::rasterize_triangle(const Triangle& t)
{
    ...
    int x0 = std::floor(min_x);
    int x1 = std::ceil(max_x);
    int y0 = std::floor(min_y);
    int y1 = std::ceil(max_y);
    ...
    for (int x = x0; x <= x1; x++)
    {
        for (int y = y0; y <= y1; y++)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                float sx = x + sample_offsets[i].x();
                float sy = y + sample_offsets[i].y();
                ...
                if (insideTriangle(sx, sy, t.v))
                {
                    ...
                    int sample_index = pixel_index * 4 + i;

                    if (z_interpolated < ssaa_depth_buf[sample_index])
                    {
                        ssaa_depth_buf[sample_index] = z_interpolated;
                        ssaa_frame_buf[sample_index] = t.getColor();
                    }
                }
            }

            Eigen::Vector3f color =
                (ssaa_frame_buf[pixel_index * 4 + 0] +
                    ssaa_frame_buf[pixel_index * 4 + 1] +
                    ssaa_frame_buf[pixel_index * 4 + 2] +
                    ssaa_frame_buf[pixel_index * 4 + 3]) / 4.0f;
...
void rst::rasterizer::clear(rst::Buffers buff)
{
    if ((buff & rst::Buffers::Color) == rst::Buffers::Color)
    {
        ...
        std::fill(ssaa_frame_buf.begin(), ssaa_frame_buf.end(),
    ...
    if ((buff & rst::Buffers::Depth) == rst::Buffers::Depth)
    {
        ...
        std::fill(ssaa_depth_buf.begin(), ssaa_depth_buf.end(),
            std::numeric_limits<float>::infinity());
    }
}
